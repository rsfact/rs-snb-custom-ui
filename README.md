...
SNB メイン画面（ボード）
  ...
  └─ ③ 「サンプル」フォルダを作成 → 書類をアップロード → クエリをコピー
       └─ ④ カスタムサブUI（index.html）を開き、クエリを貼り付け
...
| 5 | **「サンプル」フォルダ** | フォルダ **「サンプル」** を新規作成 → 書類を1枚アップロード → クエリをコピー |
| 6 | **クエリを貼り付け** | アドレスバー末尾にクエリを貼って Enter → ログイン → **表示できたか AI に報告** |
...
1. Dify で import したワークフローを **公開** する（未公開だと API キーが使えない）
2. Dify で「API アクセス」から **ベース URL** と **API キー** を取得する
2. SNB のコンソールで Rule を作成する。AI が案内する内容:
   - **ルール名**（例: `名刺読み取り`）
...
1. SNB にログインし、アップロード画面（Rule に合う board）を開く
2. **「サンプル」** という名前の **フォルダを新規作成** する
3. 作成した **「サンプル」フォルダを開き**、右下の **「+」** から試し用の書類（PNG / JPEG）を **1枚アップロード** する
4. 読み取りが終わるまで待つ
5. 「サンプル」フォルダのカード左上にマウスを乗せ、**クエリ**（`?board_id=...&task_id=...`）をクリックしてコピーする

### 6. クエリを貼り付けて確認

1. 手順4で開いたブラウザの **アドレスバー末尾** に、コピーしたクエリを貼り付けて Enter

   例:

   ```
   file:///C:/Users/あなた/Downloads/scanners-base-sub-ui/frontend/index.html?board_id=meishi-001&task_id=xyz
   ```

2. ログイン画面が出たら、SNB と同じメールアドレス・パスワードでログインする
3. 読み取り結果が画面に表示される
4. AI に **「うまく表示されましたか？」** と聞かれるので、結果を伝える
...
| 「サンプル」フォルダ | ローカル確認用。フォルダ名は **サンプル** で新規作成し、書類を1枚アップロード |
