...
SNB メイン画面（ボード）
  ...
  └─ ③ 「サンプル」フォルダを作成 → 書類をアップロード
       └─ ④ 専用コードをコピーし、開いている index.html の末尾に貼り付け
...
| ツール |  |
|---|---|
| **Claude Code** | 標準。GitHub のこのリポジトリを開いて対話する |
| **Cursor** | 利用可。ワークスペースにリポジトリを clone して `AGENT.md` を参照 |
| **Gemini 等** | 本手順の対象外 |

AI は GitHub から `AGENT.md`・`sample/`・`dify.yml` を **直接読めます**。コードをチャットに貼り付ける必要はありません。
...
| # | 段階 | 内容 |
|---|---|---|
| 0 | **リポジトリ + URL** | GitHub で本リポジトリを開き（Claude Code / Cursor）、Scanners Base の URL を AI に伝える |
...
| 6 | **専用コードを貼り付け** | 右下ペーパークリップから専用コードをコピー → 開いている **`index.html` の末尾に貼り付け** → Enter → 表示確認を報告 |
| 7 | 調整・完成 | 表示 OK なら2ファイルをエンジニアへ / NG なら AI と解決 → 修正 → F5 |

---

### 0. リポジトリを開いて AI と対話する

1. GitHub の **scanners-base-custom-ui** リポジトリを開く
2. **Claude Code** または **Cursor** で、リポジトリをワークスペースとして開き、`AGENT.md` を読ませながら対話する
3. AI が最初に Scanners Base の URL を聞くので、ブラウザのアドレスバーに表示されている URL を教える
...
2. AI が `index.html` と `login.html` を出力する（ベースはリポジトリの `sample/`）
3. 出力を **`index.html` と `login.html` として保存**する（Claude Code / Cursor なら `sample/` 内のファイルをそのまま更新してもよい）
...
4. 読み取りが終わるまで待つ
5. **「サンプル」** フォルダの **右下** にある **ペーパークリップ**（📎）をクリックし、**専用コード** をコピーする
6. いま開いている **`index.html` のブラウザのアドレスバー末尾** に、コピーした専用コードを貼り付けて **Enter** する
...
3. AI が出した新しいコードで `index.html` を更新する（リポジトリ上の `sample/index.html` を直してもよい）
4. ブラウザを更新（F5）して確認する
5. 満足いくまで繰り返す
...
| 専用コード | `?board_id=xxx&task_id=yyy` の形式。「サンプル」フォルダ **右下のペーパークリップ** からコピーし、**開いている index.html の末尾** に貼り付ける |
