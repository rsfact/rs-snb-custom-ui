...
4. Dify 作成と Rule 設定を案内する:

> では、この output 構成で Dify のワークフローを作成してください。  
> ワークフローの構造化出力は、次の JSON 形式にしてください。  
> （確定した JSON 例を提示）
>
> Dify ができたら、SNB の Rule も設定してください。最低限、次の4つを入力します。
>
> | 項目 | 内容 |
> |---|---|
> | **name** | Rule の名前 |
> | **Dify URL** | Dify の API ベース URL |
> | **Dify API キー** | Dify ワークフローの API キー |
> | **正規表現** | ボード ID にマッチする正規表現 |
>
> **Dify と Rule の設定が終わったら教えてください。**  
> 試しに1枚アップロードして、読み取り結果が想定どおり返ることを確認してから、画面の作成に進みましょう。

5. ユーザーが「組んだ」「設定した」と返すまで待ってもよい（先に UI を進めたいと言えば進めてよい）
...
3. **Dify は組まれているか聞く**（必須・URL の直後）
   ...
   - **Dify 未設定** → output スキーマを対話で確定 → Dify 作成 + Rule 設定を案内 →「組んだら教えて」
